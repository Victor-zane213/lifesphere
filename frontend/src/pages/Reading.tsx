...
export default function Reading() {
  ...
  function load() {
    readingsApi.list().then(setItems).catch(() => {});
  }
  ...
  async function handleSave() {
    ...
    if (editing) {
      await readingsApi.update(editing.id, { ...editing, title: formTitle.trim(), content: formContent.trim() });
    } else {
      await readingsApi.create({ title: formTitle.trim(), content: formContent.trim() });
    }
    setShowForm(false);
    load();
  }
    ...
    <div className="min-h-screen" style={{ paddingTop: '60px' }}>
      <div className="mx-auto px-6 py-10" style={{ maxWidth: '1200px' }}>
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-xl font-light" style={{ color: '#333' }}>我的阅读</h1>
          <button
            onClick={openNew}
        ...
        <div className="grid gap-4">
          {items.map((item) => (
            <ContentCard key={item.id} onEdit={() => openEdit(item)} onDelete={() => handleDelete(item.id)}>
              ...
              {item.content && <p className="text-sm leading-relaxed" style={{ color: '#555' }}>{item.content}</p>}
              {item.created_at && (
                <p className="text-xs mt-2" style={{ color: '#aaa' }}>{new Date(item.created_at).toLocaleDateString('zh-CN')}</p>
              )}
            </ContentCard>
          ))}
          {items.length === 0 && <p className="text-sm" style={{ color: '#aaa' }}>暂无阅读记录</p>}
        </div>
      </div>
    </div>
  );
